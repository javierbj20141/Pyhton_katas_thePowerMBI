...
def factorial (numero):
  if numero == 0 or numero == 1: ##establecemos caso base cuando el numero acabe siendo 1 o 0, nos devuelva un 1
    return 1
  else:
    return numero * factorial (numero - 1) ##aqui actua la recursión, cada vez va multiplicando el resultado por un numero menos, hasta que llegue a 1
print (factorial(4))
...
# %%
lista_tupla = [("Hola me gusta Python",34,2356), ("Soy del Real Madrid",45,True)]
def lista_string (tupla): 
    lista = list(map(str,tupla)) ##hacemos que las tuplas se vulvan texto, y lo convertimos en lista
    str_unidos = " ".join (lista)##hacemos que esa lista desaparezca y sus valores se separen por espacios
    return str_unidos
print(list(map(lista_string,lista_tupla)))##para aplicar esta funcion siempre necesitaremos la funcion map, y asi creamos una lista cn los vaores de la tupla que pasan a texto separado por espacios con el metodo join
...
def masoctas_validas (lista):
    mascotas_prhibidas = ["Mapache", "Tigre","Serpiente Pitón", "Cocodrilo", "Oso"] ##indicamos las mascotas prohibidas en una lista
    return list(filter(lambda mascota:mascota not in mascotas_prhibidas,lista)) ##indicamos una funcion lambda que nos de una lista filtrada por los parametros que no aparezcan en la lista de mascotas prohibidas
print(masoctas_validas(lista_mascotas))
...
lista_numero = [5,7,2]
lista_numerov2 = [6,7,2]

def resultado (lista):
      return int(reduce (lambda x,t: x*10+t,lista))##lo que hace es que 5*10+7 = 57, se conviete de nuevo en x, despues hace 570 + 2 y ya te da 572
print (resultado(lista_numero))
##otra manera según comentarios
def resultadov2 (lista):
      strings = list(map(str,lista)) ##pasamos a str los valores de la lista
      definitivo = "".join(strings) ##pasamos la lista a una cadena, en este caso sin que sus valores estén separados por espacios
      return int(definitivo)##que nos devuelva esa cadena en formato int
print(resultadov2(lista_numerov2)) 
...
class arbol:
    ...
    def quitar_rama(self,posicion):##se indica la posición como parametro, para poder eliminar la posición que quieras
        self.ramas.pop(posicion) ##con el metodo pop eliminas la posición que quieras
        return (f"Se ha eliminado la rama de la posición 2. Ramas totales : {self.ramas}")
    def info_arbol(self):
        return (f"información: su tronco mide {self.tronco}, tiene {len(self.ramas)} ramas y sus longitudes son {", ".join(list(map(str,self.ramas)))}")      
...
class UsuarioBanco:
    ...
    def retirar_dinero(self,cantidad):
        if cantidad <= self.saldo:##hacemos que la cantidad, en caso de que sea menor que el dinero en la cuenta bancaria, se retire.
            self.saldo -= cantidad
    ...
    def agregar_dinero(self,dinero):##agregar dinero al saldo actual, añadiendo el parámetro de dinero
    ...
    def transferir_dinero(self,otro_usuario,amount):
        if self.saldo >= amount:
            self.saldo -= amount##la cantidad se reira de un usuario
            otro_usuario.saldo += amount ##y aumenta esa cantidad en el otro usuario, que se indica a través de un parametro 
...
def notas (clasificacion):
    ...
    if clasificacion >=0 and clasificacion <=69: ##vamos poniendo las condiciones para que de las notas segun clasificacion
        return "Insuficiente"
    elif clasificacion >= 70 and clasificacion <=79:
        return "Bien"
    elif clasificacion >=80 and clasificacion <=89:
        return "Muy bien"
    elif clasificacion >=90 and clasificacion <=100:
        return "Excelente"
    else:
